...
macro_rules! m {
    ...
    (atom ($t:tt) $elongate:literal $($ts:tt)*) => {{
        ...
        let elongated = $crate::mini::Chain(core::iter::once(p).chain(silence));
    ...
    (group $l:literal $($ts:tt)*) => {{
        ...
        let ps = m!(group $($ts)*);
        $crate::mini::Chain(core::iter::once(p).chain(ps))
    }};
    (group [$($a:tt)*], [$($b:tt)*]) => {{
...
// Provides an `ExactSizeIterator` implementation `for core::iter::Chain`
// The only reason it's omitted from the core library is that the sum of
// the two lengths might overflow `usize`... To account for this, we check
// for the upper bound on the size_hint explicitly.
pub struct Chain<A, B>(pub core::iter::Chain<A, B>);
